    ...
    const { error: emailError } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: 'Courier New', monospace; background: #050a12; color: #fff; padding: 40px; border-radius: 4px; max-width: 600px;">
          ...
          <p style="color: rgba(255,255,255,0.6); font-size: 14px; line-height: 1.8;">Dear ${customerName},</p>
          
          <div style="margin: 24px 0; padding: 24px; background: rgba(200,169,110,0.05); border-left: 3px solid #c8a96e;">
            <p style="color: rgba(255,255,255,0.8); font-size: 14px; line-height: 1.8; margin: 0;">${replyMessage.replace(/\n/g, '<br/>')}</p>
          </div>
          ...
          <div style="background: rgba(255,255,255,0.03); padding: 16px; border-radius: 2px;">
            ...
            <p style="color: rgba(255,255,255,0.4); font-size: 13px; font-style: italic; margin: 0;">${originalMessage.replace(/\n/g, '<br/>')}</p>
          </div>
        </div>
      `,
    });
